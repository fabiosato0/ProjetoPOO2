...
package controller;
import java.sql. *;
import model.Administrador; // Trocado de Usuario para Admin
...
public class ControladorAdmin { // Trocado de ControladorUsuario para ControladorAdmin
    ...
    public void deletar(int idAdmin){ // Trocado de idCliente para idAdmin
        ...
        String sql2 = "DELETE FROM administrador WHERE idadmin = ?"; // Trocado 'usuario' e 'idcliente'
    ...
    public List<Administrador> listarTodos() { // Trocado de Usuario para Admin
        // A query correta para buscar dados de admin
        PreparedStatement ps = null;
        String sql = "SELECT * FROM administrador"; // Assumindo que a tabela é 'admin'

        // Crie uma lista vazia para guardar os admins encontrados
        List<Administrador> adminsEncontrados = new ArrayList<>(); // Trocado
        ...
        try{
            ...
            while (rs.next()) {
                ...
                Administrador admin = new Administrador(); // Trocado
    ...
    public void modificar(Administrador admin) { // Trocado de Usuario para Admin
    PreparedStatement ps = null;
    ...
    String sql3 = "UPDATE administrador SET nome = ?, cpf = ?, telefone = ?, idade = ? WHERE idadmin = ?"; // Trocado 'usuario' e 'idcliente'
